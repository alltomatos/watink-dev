...
// extractToken reads Bearer token from Authorization header or ?token query param.
...
// parseUserFromToken validates JWT and extracts userID, profile, tenantID.
...
// hasSwaggerGroupPermission checks if the user's group grants swagger access.
// Uses injected DB — no global singleton.
func (sc *SwaggerController) hasSwaggerGroupPermission(userID int, tenantID string) bool {
	...
	var count int64
	sc.db.Table("GroupPermissions gp").
...
// ensureSwaggerAccess validates token and checks swagger permission.
// Returns true if access is granted; sends HTTP error and returns false otherwise.
