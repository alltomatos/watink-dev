...
# Watink Business Interactive Installer
# Uso: sudo bash watink-business.sh

GITHUB_BIN_REPO="alltomatos/watink-business"
...
require_root() {
  if [[ "${EUID}" -ne 0 ]]; then
    err "Execute como root: sudo bash watink-business.sh"
    exit 1
  fi
}
...
[Unit]
Description=Watink Business Core
...
write_nginx_conf() {
  ...
  cat >/etc/nginx/sites-available/watink-business.conf <<EOF
server {
  listen 80;
  server_name ${domain};
...
EOF
  ...
  ln -sf /etc/nginx/sites-available/watink-business.conf /etc/nginx/sites-enabled/watink-business.conf
...
main() {
  ...
  echo
  echo "======================================================"
  echo "   Watink Business - Instalador Interativo"
  echo "======================================================"
  echo
